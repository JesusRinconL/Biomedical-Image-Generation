...
if __name__ == '__main__':
    ...
    NUM_labels = 8 # Modify for each case
    ...
    image_LABELdirectory = os.path.join(main_dir, "datasets_patrones\\DS1__6capas_Vir")
    dataset_path = os.path.join(main_dir,"DataSet__6capas_Vir")
    path_test = os.path.join(dataset_path,"test")
    path_train = os.path.join(dataset_path,"train")
    path_val = os.path.join(dataset_path,"val")

    used_images_file = os.path.join(os.path.dirname(main_dir), "used_images10.plk")
    ...
    for ii in range(len(jpg_files)):
        ...
        if not (jpg_files[ii] in used_images):
            ...
            if os.path.exists(image_file) & os.path.exists(labelsMASKfile):
                ...
                Origins = randomOrigins(HEIGHT_image, WIDTH_image, HEIGHT, WIDTH, np.linalg.norm([HEIGHT, WIDTH]),6)
