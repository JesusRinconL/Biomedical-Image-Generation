...
# CARPETA DE SALIDA
image_LABELdirectory = "C:\\Users\\Jesus\\Documents\\TFG\\PROYECTO2\\GenerarDatasets\\datasets_patrones\\DS1__8capas"
    ...
    if not os.path.exists(labelsMASKfile):
        if os.path.exists(healingMASKfile) & os.path.exists(nucleiMASKfile) & os.path.exists(cytoMASKfile):
            ...
            if not ejecutado_capas:
                ...
                label_mean_between = label_mean[(label_mean >= lower_bound_1) & (label_mean < upper_bound_1)]
                sorted_means_between = np.sort(label_mean_between)
                # Divide el rango de medias en tres partes iguales
                lower_bound_2 = sorted_means_between[int(len(sorted_means_between) / 3)]
                upper_bound_2 = sorted_means_between[int(2 * len(sorted_means_between) / 3)]
                ejecutado_capas = True
                print("EL valor del límite inferior es", round(lower_bound_2, 2), "y del límite superior", round(upper_bound_2, 2))
                
            # Utiliza los límites para clasificar las etiquetas en cinco capas
            labels1stLayer = labels[label_mean < lower_bound_1]
            labels2ndLayer = labels[(label_mean >= lower_bound_1) & (label_mean < lower_bound_2)]
            labels3rdLayer = labels[(label_mean >= lower_bound_2) & (label_mean < upper_bound_2)]
            labels4thLayer = labels[(label_mean >= upper_bound_2) & (label_mean < upper_bound_1)]
            labels5thLayer = labels[label_mean >= upper_bound_1]
             

            cytoMASK1stLayer = cytoMASK2.copy()
            cytoMASK1stLayer[np.isin(cytoMASK1stLayer, np.hstack((labels2ndLayer, labels3rdLayer, labels4thLayer, labels5thLayer)))] = 0
            
            cytoMASK2ndLayer = cytoMASK2.copy()
            cytoMASK2ndLayer[np.isin(cytoMASK2ndLayer, np.hstack((labels1stLayer, labels3rdLayer, labels4thLayer, labels5thLayer)))] = 0
            
            cytoMASK3rdLayer = cytoMASK2.copy()
            cytoMASK3rdLayer[np.isin(cytoMASK3rdLayer, np.hstack((labels1stLayer, labels2ndLayer, labels4thLayer, labels5thLayer)))] = 0
            
            cytoMASK4thLayer = cytoMASK2.copy()
            cytoMASK4thLayer[np.isin(cytoMASK4thLayer, np.hstack((labels1stLayer, labels2ndLayer, labels3rdLayer, labels5thLayer)))] = 0
            
            cytoMASK5thLayer = cytoMASK2.copy()
            cytoMASK5thLayer[np.isin(cytoMASK5thLayer, np.hstack((labels1stLayer, labels2ndLayer, labels3rdLayer, labels4thLayer)))] = 0
            ...
            imageLABELS[(cytoMASK4thLayer != 0) & (~boundary_maskCYTO)] = 5
            imageLABELS[(cytoMASK5thLayer != 0) & (~boundary_maskCYTO)] = 6
            imageLABELS[(boundary_maskCYTO != 0) & (~maskHEALING)] = 7
            imageLABELS[(masksNUCLEI != 0) & (~boundary_maskNUCLEI)] = 8
            imageLABELS[boundary_maskNUCLEI] = 9
